"""
train_acdc_1ch.py — Train UNetResNet34 on ACDC with 1-channel input
=====================================================================
Channel 1: MRI image only (no segmentation mask)

Train on: data/acdc-cleaned/train_split/  (160 volumes, patients 001-080)
Test on:  data/lv-landmark/Testing/       (100 volumes, separate institution)

This is Experiment 1 — baseline that tests whether training on ACDC
alone (without seg mask) improves RV insertion point detection on
an unseen clinical dataset. Compare against train_acdc_2ch.py to
isolate the contribution of segmentation mask fusion.

Usage:
    python train_acdc_1ch.py
    python train_acdc_1ch.py --p2-checkpoint checkpoints/acdc_1ch_.../best_p2.pth
"""
...
def train(p2_checkpoint=None):
    ...
    print(f"In channels: {IN_CHANNELS} (MRI only — no seg mask)")
    print(f"Experiment : ACDC 1-channel baseline")
    ...
    print(f"Total slices: {n_total} | Train: {len(train_idx)} | Val: {len(val_idx)}")

    # ── build model with 2 input channels ────────────────────────────────────
    model = UNetResNet34(
        in_channels=IN_CHANNELS,
        num_classes=2,
        dropout=0.2,
    ...
    run_dir   = os.path.join("checkpoints", f"acdc_1ch_{ts}")
    ...
    if p2_checkpoint is None:
        ...
        # ═══════════════════════════════════════════════════════════
        print(f"\n{'='*60}\n  PHASE 1 — warmup ({P1_EPOCHS} ep, frozen enc)\n{'='*60}")
        ...
        for ep in range(1, P1_EPOCHS + 1):
            ...
            tl_loss, _ = train_epoch(model, tl, op1, crit, device,
                                     SIGMA_P1, do_mixup=True, do_aux=has_aux, aw=aw)
            v = validate(model, vl, crit, device, SIGMA_P1)
            sc1.step()
            print(f"[P1] {ep:2d}/{P1_EPOCHS} | sigma={SIGMA_P1} | "
            ...
            history.append({"epoch": ep, "train_loss": tl_loss,
        ...
        # ═══════════════════════════════════════════════════════════
        print(f"\n{'='*60}\n  PHASE 2 — curriculum ({P2_EPOCHS} ep)\n{'='*60}")
        ...
        for ep_loc in range(1, P2_EPOCHS + 1):
            ...
            aw = aux_weight(sigma)
            tl_loss, _ = train_epoch(model, tl, op2, crit, device,
            ...
            print(f"[P2] {ep_g:2d} | sigma={sigma:.2f} | "
                  f"Train={tl_loss:.4f} | Val={v['val_loss']:.4f} | "
                  f"MRE={v['mre']:.2f}px (LM1={v['mre1']:.2f} LM2={v['mre2']:.2f}) | "
                  f"SDR@2/5/10: {v['sdr'][2]:.3f}/{v['sdr'][5]:.3f}/{v['sdr'][10]:.3f} | "
                  f"P50={v['pct'][50]:.2f} P90={v['pct'][90]:.2f} Max={v['pct'][100]:.2f}px"
                  + (" ✓ best" if improved else ""))
            history.append({"epoch": ep_g, "train_loss": tl_loss,
                            "val_loss": v["val_loss"], "mre": v["mre"],
    ...
    crit_p3 = HeatmapLoss(
        coord_weight=25.0, sep_weight=0.3, sep_min_dist=0.08,
        wing_w=0.008, wing_eps=0.002, lm_weights=LM_WEIGHTS, hard_k=HARD_K,
    )
    ...
    for ep_loc in range(1, P3_EPOCHS + 1):
        ...
        tl_loss, _ = train_epoch(model, tl, op3, crit_p3, device,
                                 sigma, do_mixup=False, do_aux=False, aw=0.0, clip=3.0)
        v = validate(model, vl, crit_p3, device, sigma)
        sc3.step()

        improved = v["pct"][90] < best_mre_p3
        print(f"[P3] {ep_g:2d} | sigma={sigma:.2f} | "
              f"Train={tl_loss:.4f} | Val={v['val_loss']:.4f} | "
              f"MRE={v['mre']:.2f}px (LM1={v['mre1']:.2f} LM2={v['mre2']:.2f}) | "
              f"SDR@2/5/10: {v['sdr'][2]:.3f}/{v['sdr'][5]:.3f}/{v['sdr'][10]:.3f} | "
              f"P50={v['pct'][50]:.2f} P90={v['pct'][90]:.2f} Max={v['pct'][100]:.2f}px"
    ...
    # ── best SDR@5 from history ───────────────────────────────────────────────
    best_sdr5   = max((h["sdr"]  for h in history), default=0.0)
    best_mre_all = min((h["mre"] for h in history), default=float("inf"))
    ...
    results = {
        "run_dir":      run_dir,
        "in_channels":  IN_CHANNELS,
        "best_p90_mre": round(best_mre_p3, 4),
        "best_mre":     round(best_mre_all, 4),
        "best_sdr5":    round(best_sdr5, 4),
        ...
        "config": {
            ...
            "lr_backbone_p2":  LR_BACKBONE_P2,
            "lr_p3":           LR_P3,
            "sigma_p1":        SIGMA_P1,
            "sigma_end":       SIGMA_P3_END,
            "weight_decay":    WEIGHT_DECAY,
            "epochs_p1":       P1_EPOCHS,
            "epochs_p2":       P2_EPOCHS,
            "epochs_p3":       P3_EPOCHS,
    ...
    print(f"  Best P90 MRE  : {best_mre_p3:.2f}px")
    print(f"  Best MRE      : {best_mre_all:.2f}px")
    print(f"  Best SDR@5px  : {best_sdr5:.1%}")
    print(f"  Total epochs  : {final_epoch}")
    print(f"  Checkpoint    : {best_ckpt}")
    print(f"{'='*44}")
    print(f"\nResults JSON : {results_path}")
    print(f"\nEvaluate with:")
    print(f"  python inference_acdc.py --checkpoint {best_ckpt}")
