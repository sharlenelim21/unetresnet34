"""
finetune_mixed.py — Cross-domain fine-tuning with mixed training data
=====================================================================
Starts from an ACDC checkpoint and fine-tunes on a 50/50 mix of:
  - rv_landmark training slices
  - ACDC training slices (patients 001-080)

Training uses the same 3-phase curriculum as finetune_rv.py:
...
Validation uses rv_landmark val split only (patient-level 80/20 split).
Test set (data/rv_landmark/test_*) is NEVER touched during training.

Checkpoint criterion: P90 MRE on rv_landmark validation set.
...
def set_sigma_on_datasets(concat_ds, sigma):
    """Call set_sigma on each sub-dataset within a ConcatDataset."""
    for ds in concat_ds.datasets:
        if hasattr(ds, "set_sigma"):
            ds.set_sigma(sigma)
...
def train_one_epoch(model, loader, optimizer, criterion, device, scaler=None):
    ...
    Returns (avg_loss, sub_avgs, n_steps, n_skipped) where n_steps counts
    iterations where the optimizer actually stepped.
    """
    model.train()
    losses = 0.0
    n_loss = 0
...
def run_phase(
    ...
    for ep_in_phase in range(n_epochs):
        ...
        set_sigma_on_datasets(train_ds, sigma)
...
def _enforce_and_match_np(pred_np, gt_np):
    """Enforce superior ordering then pick normal vs swapped assignment."""
    p = pred_np.copy()
    if p[1] > p[3]:
...
def evaluate_test(model, in_channels, device):
    """
    Run TTA inference on the rv_landmark test set and return metric dict.
    GT is loaded via RVLandmarkDataset (combined-heatmap blob extraction).
    """
    ...
    for images, _, gt_coords in loader:
        ...
        for t in total_sdr:
            total_sdr[t] += s[t]
        sample_mres.extend(compute_per_sample_mre(pc_matched, gt_coords).cpu().tolist())
...
def main():
    ...
    run_dir = os.path.join("rv-checkpoints",
                           f"finetune_mixed_{args.in_channels}ch_{ts}")
    ...
    # ── patient-level split of rv_landmark (val uses rv_landmark only)
    train_files, val_files = split_volumes(IMAGE_DIR, val_frac=args.val_frac, seed=SEED)
    print(f"RV train volumes: {len(train_files)}   Val volumes: {len(val_files)}")

    seg_dir = SEG_DIR if args.in_channels == 2 else None

    rv_ds = RVLandmarkDataset(
    ...
    train_ds = ConcatDataset([rv_ds, acdc_ds])
    print(f"Mixed train set: {len(rv_ds)} rv_landmark + {len(acdc_ds)} ACDC = {len(train_ds)} slices")
    ...
    model = UNetResNet34(in_channels=args.in_channels, num_classes=2,
                         ...
                         use_instance_norm=args.instance_norm).to(device)
    ...
    print("\n[baseline] validating checkpoint on rv_landmark val split")
    ...
    config = {
        ...
        "rv_train_volumes": train_files,
        "val_volumes":      val_files,
        "acdc_train_ids":   ACDC_TRAIN_IDS,
        "rv_train_slices":  len(rv_ds),
        "acdc_train_slices": len(acdc_ds),
        "best_p90": best_p90_ref[0],
    }
    ...
    # ── test evaluation on rv_landmark test set
    ...
    if test_m is not None:
        ...
        print(f"\n{'='*44}")
        print("FINE-TUNING COMPLETE (mixed training)")
