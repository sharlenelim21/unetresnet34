"""
train_acdc_2ch.py — Train UNetResNet34 on ACDC with 2-channel input
=====================================================================
Channel 1: MRI image
Channel 2: Cardiac segmentation mask (LV/RV/Myo)
           Blank (zeros) when RV is absent from slice (apex/base)
...
This is Experiment 2 — tests whether seg mask fusion improves
RV insertion point detection on an unseen clinical dataset.

Usage:
    python train_acdc_2ch.py
    python train_acdc_2ch.py --p2-checkpoint checkpoints/acdc_2ch_.../best_p2.pth
"""
...
def train(p2_checkpoint=None):
    ...
    print(f"In channels: {IN_CHANNELS} (MRI + seg mask, blank when no RV)")
    print(f"Experiment : ACDC 2-channel seg fusion")
    ...
    run_dir   = os.path.join("checkpoints", f"acdc_2ch_{ts}")
