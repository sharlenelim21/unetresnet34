...
def train(p2_checkpoint=None, use_instance_norm=False, min_lm1_confidence=0.0,
    ...
    run_tag = ("acdc_2ch_instnorm" if use_instance_norm
               else "acdc_2ch_groupnorm" if use_group_norm
               else "acdc_2ch_filtered" if min_lm1_confidence > 0
               else RUN_TAG)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--instance-norm", action="store_true",
                        help="Use InstanceNorm in encoder (replaces BatchNorm in enc0-enc4)")
    parser.add_argument("--min-lm1-confidence", type=float, default=0.0,
                        help="Filter slices where LM1 pixel count / 5 < threshold (0=off)")
    ...
    try:
        train(p2_checkpoint=args.p2_checkpoint,
              ...
              min_lm1_confidence=args.min_lm1_confidence,
